...
| **Q3 dlink 吞吐** | ⚠️ **按实际业务量级收口，不再测 GB 级**（业务上传一般没有那么大的文件）。17.5 MiB / 5 文件的端到端约 **276 KiB/s**：遍历分享 ~20s + 转存 0.9s + 下载 44.5s ≈ 65s。单文件方差极大（11.0 MiB 跑 280 KiB/s，6.4 MiB 跑 4.2 MiB/s，差 15 倍），原因未查明。 |
...
**Q3 的实际含义**：这个量级下**固定开销占大头**——光递归遍历分享就约 20s（21 个目录，
目录越深越慢），每个文件下载另有约 0.9s 的固定延迟，都与文件大小无关。因此对几 MB
到几十 MB 的文件，网盘同步能否胜过直传，取决于客户的**上行带宽**：低于约 2.2 Mbps
才划算。这条路更实在的价值可能在**流程**（客户随时往网盘丢、平台按需拉，不必守着
浏览器等上传）而非纯速度——是否推广给某个客户，按其网络状况判断。

**仍待完成**：Q4 用 100+ 文件试探单次转存的真实上限（当前 `netdisk_pull_batch=100`
是保守猜测，未验证）。
