...
### 转换期的静默与 socket hang up（2026-07-30 定位，已修）

一次 25.5 MB STEP 的轻量化以 `socket hang up` 告终。门户侧证据：源文件已被完整
取走（nginx 记 26,787,727 字节 / 200，与磁盘一致），产物回传请求**从未到达**，
nginx 错误日志为空。在 vektor3d 源码侧用同一文件复现后，根因是**连接复用**：

Node ≥19 的 `http.globalAgent` 默认 keepAlive，下载完成后 socket 留在池里；
紧接着的 STEP→3dix 转换是**同步阻塞事件循环**的进程内 WASM 计算——实测该文件
阻塞 74.9 秒，期间 2 秒一次的定时器触发 **0** 次。阻塞使得 ① agent 自己的 5 秒
空闲清理跑不了，② 对端 FIN（nginx `keepalive_timeout` 默认 75s）也处理不了。
于是转换一结束，回传就把请求写进这条已死的连接，确定性失败且服务端毫无记录。

**已修**：`capability-server/transfer.js` 的下载与回传都改为 `agent: false`
